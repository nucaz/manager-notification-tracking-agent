...
router.post('/subir/:entityType/:entityId', canWrite, upload.single('file'), verifyCsrfToken, async (req, res, next) => {
  try {
...
router.post('/:id/aplicar', canWrite, verifyCsrfToken, async (req, res, next) => {
  try {
    const [rows] = await pool.query('SELECT * FROM attachments WHERE id = ?', [req.params.id]);
    const attachment = rows[0];
    if (!attachment) {
